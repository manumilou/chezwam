...
def parse(environ, result):
    url = result.pop('url')
    log.error("Debug result variable: %s", result)
    try:
        environ['chezwam.navigation'] = navigation_from_path(url)
    except NoPage, e:
        ...
        result['action'] = 'view'
        result['id'] = environ['chezwam.navigation']['page'].id
    return True
...
def make_map():
    ...
    # CUSTOM ROUTES HERE
    map.connect('/page/{pageid}/{controller}/{action}',
